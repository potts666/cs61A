...
def roll_dice(num_rolls, dice=six_sided):
    ...
    curr_dice = 0
    flag = 0
    while num_rolls > times:
        curr_dice = dice()
        sum_dice , times = sum_dice + curr_dice, times + 1
        if curr_dice == 1:
           flag = 1
    if flag == 1:
        return 1
...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    score, op_num, pl_num = 0, 0, 0
    op_num = (opponent_score // 10 ) % 10
    pl_num = player_score % 10
    score = 3 * abs(op_num - pl_num)
    if score == 0:
        return 1
    else:
        return score
...
def is_always_roll(strategy, goal=GOAL):
    ...
    score0 = 0
    score1 = 0
    roll_times = strategy(score0, score1)
    while(score0 < goal):
        while(score1 < goal):
            if(strategy(score0, score1) != roll_times):
                return False
            score1 += 1
        score1 = 0
        score0 += 1
...
def make_averaged(original_function, samples_count=1000):
    ...
    def sum_number(*args):
        ...
        while i < samples_count:
            points += original_function(*args)
            i += 1
        return points / samples_count
    return  sum_number
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    count = 1
    while count <= 10:
        average = make_averaged(roll_dice, samples_count)(count, dice)
        if average > maximum:
            maximum = average
            max_order = count
        count += 1 
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=8):
    ...
    # BEGIN PROBLEM 11
    if threshold <= sus_points(score + boar_brawl(score, opponent_score)) - score:
        return 0
    else:
        return num_rolls
